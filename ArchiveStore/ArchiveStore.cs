...
namespace Neo.Plugins
{
    public class ArchiveStore : Plugin, IPersistencePlugin
    {
        ...
        public static byte[] GetStorageAtHeight(byte[] key, uint height) => ExtractStorage(key, store.Seek(CompositeKey(key, height), SeekDirection.Backward).FirstOrDefault());
        ...
        public static IEnumerable<byte> UintToBytes(uint n) => BitConverter.IsLittleEndian ? BitConverter.GetBytes(n).Reverse() : BitConverter.GetBytes(n);
    }
}
